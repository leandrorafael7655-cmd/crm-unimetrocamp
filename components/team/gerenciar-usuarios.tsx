...
export function GerenciarUsuarios({ perfil }: { perfil: Usuario }) {
  ...
  const adicionar = async (e: React.FormEvent) => {
    ...
    const r = await addUser({ fullName: nome, email, role: papel, consultantTag: tag })
    mostrar(r)
    if (r.ok) {
      setNome("")
      setEmail("")
  ...
  return (
    <section className="rounded-lg border border-slate-200 bg-white p-4">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          <p className="mt-0.5 text-xs text-slate-500">
            Cadastre acessos, defina perfis, recupere senhas e remova usuários com segurança.
          </p>
        </div>
        <div className="flex items-center gap-2">
          <Link href="/auth/reset-password" className={btnNeutro}>
            Alterar minha senha
          </Link>
      ...
      {ehAdministrador ? (
        <form onSubmit={adicionar} className="mt-4 grid gap-2 sm:grid-cols-2 lg:grid-cols-5 lg:items-end">
          ...
          <button type="submit" className={btn} disabled={ocupado === "adicionar"}>
            {ocupado === "adicionar" ? "Adicionando…" : "Adicionar usuário"}
          </button>
        </form>
      ) : (
        <div className="mt-4 rounded-md border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800">
          Somente o perfil Gerente pode adicionar ou excluir usuários. A supervisão pode administrar papel, status e recuperação de acesso.
        </div>
      )}

      <p className="mt-2 text-[11px] text-slate-500">
        Ao adicionar um usuário, o sistema cria a conta no Supabase Auth e envia um e-mail para o próprio usuário definir a senha.
      </p>
        ...
        <table className="w-full min-w-[760px] text-sm">
          ...
          <tbody className="divide-y divide-slate-100">
                  ...
                  <td className="px-3 py-2">
                    <div className="flex flex-wrap justify-end gap-1.5">
                      ...
                      <button
                        ...
                        onClick={() => acao(u.id, () => sendPasswordRecovery(u.email), false)}
                      >
                        Enviar recuperação
                      </button>
                      {ehAdministrador && u.id !== perfil.id && (
                        <button type="button" className={btnDanger} disabled={busy} onClick={() => excluir(u)}>
                          Excluir
                        </button>
                      )}
                    </div>
            ...
            {!carregando && linhas.length === 0 && (
              <tr>
                <td colSpan={4} className="px-3 py-4 text-center text-xs text-slate-400">Nenhum usuário cadastrado.</td>
              </tr>
            )}
          </tbody>
        </table>
      ...
      {ehAdministrador && (
        <div className="mt-4 rounded-md border border-dashed border-slate-300 p-3">
          <div className="flex items-center justify-between gap-3">
            ...
            <button type="button" className={btnNeutro} onClick={rodarDiag} disabled={diagLoading}>
