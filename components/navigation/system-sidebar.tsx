...
function LegacyButton({
  ...
  if (embedded && onLegacySelect) {
    return (
      <button
        type="button"
        onClick={() => onLegacySelect({ view, focus })}
        className={submenuClass(active)}
      >
...
export function SystemSidebarMenu({
  role,
  embedded = false,
  activeLegacyView = "painel",
  onLegacySelect,
}: SystemSidebarMenuProps) {
      ...
      <div className="space-y-1.5">
        {allowedGroups.includes("b2b") && (
          <section>
            <button
              type="button"
              aria-expanded={openGroup === "b2b"}
              onClick={() => toggle("b2b")}
              className={`flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold transition ${
                openGroup === "b2b" ? "bg-white/10 text-white" : "text-[#b4fcf1] hover:bg-white/5 hover:text-white"
              }`}
            >
              <Building2 className="h-4 w-4 shrink-0" />
              <span className="flex-1 text-left">{GROUP_LABELS.b2b}</span>
            ...
            {openGroup === "b2b" && (
              <div className="ml-3 mt-1 space-y-0.5 border-l border-white/10 pl-2">
                ...
                {isManager && (
                  <LegacyButton label="Todas as empresas" view="empresas" Icone={Building2} active={pathname === "/" && activeLegacyView === "empresas"} embedded={embedded} onLegacySelect={onLegacySelect} />
                )}
        ...
        {allowedGroups.includes("high-school") && (
          <section>
            <button
              type="button"
              aria-expanded={openGroup === "high-school"}
              onClick={() => toggle("high-school")}
              className={`flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold transition ${
                openGroup === "high-school" ? "bg-white/10 text-white" : "text-[#b4fcf1] hover:bg-white/5 hover:text-white"
              }`}
            >
              <GraduationCap className="h-4 w-4 shrink-0" />
              <span className="flex-1 text-left">{GROUP_LABELS["high-school"]}</span>
            ...
            {openGroup === "high-school" && (
              <div className="ml-3 mt-1 space-y-0.5 border-l border-white/10 pl-2">
                ...
                {can(canonicalRole, "supervest.read") && (
                  <NavLink href="/supervest" label="SuperVestibular" Icone={GraduationCap} active={pathname.startsWith("/supervest")} />
                )}
              </div>
            )}
          </section>
        ...
        {allowedGroups.includes("routes") && (
          <section>
            <button
              type="button"
              aria-expanded={openGroup === "routes"}
              onClick={() => toggle("routes")}
              className={`flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold transition ${
                openGroup === "routes" ? "bg-white/10 text-white" : "text-[#b4fcf1] hover:bg-white/5 hover:text-white"
              }`}
            >
              <Route className="h-4 w-4 shrink-0" />
              <span className="flex-1 text-left">{GROUP_LABELS.routes}</span>
              <ChevronDown className={`h-4 w-4 transition-transform ${openGroup === "routes" ? "rotate-180" : ""}`} />
            </button>
            {openGroup === "routes" && (
              <div className="ml-3 mt-1 space-y-0.5 border-l border-white/10 pl-2">
                {can(canonicalRole, "map.read") && (
                  <NavLink href="/mapa" label="Mapa de locais" Icone={MapPinned} active={pathname === "/mapa"} />
                )}
                {can(canonicalRole, "routes.plan") && (
                  <NavLink href="/mapa/rotas" label="Planejar rotas" Icone={Route} active={pathname.startsWith("/mapa/rotas")} />
                )}
              </div>
            )}
          </section>
        ...
        {allowedGroups.includes("settings") && (
          <section>
            <button
              ...
              className={`flex w-full items-center gap-2 rounded-lg px-3 py-2.5 text-sm font-semibold transition ${
                openGroup === "settings" ? "bg-white/10 text-white" : "text-[#b4fcf1] hover:bg-white/5 hover:text-white"
              }`}
            >
              <Settings className="h-4 w-4 shrink-0" />
              <span className="flex-1 text-left">{GROUP_LABELS.settings}</span>
              <ChevronDown className={`h-4 w-4 transition-transform ${openGroup === "settings" ? "rotate-180" : ""}`} />
            </button>
            {openGroup === "settings" && (
              <div className="ml-3 mt-1 space-y-0.5 border-l border-white/10 pl-2">
                {(can(canonicalRole, "goals.read.own") || can(canonicalRole, "goals.read.all")) && (
                  <NavLink href="/gestao/metas" label="Central de Metas" Icone={Target} active={pathname.startsWith("/gestao/metas")} />
                )}
                {isManager && (
                  <>
...
export function SystemSidebar({ role, userName }: { role: string; userName?: string | null }) {
    ...
    <nav className="flex w-full shrink-0 flex-col bg-[#00302b] text-white md:min-h-screen md:w-64">
      ...
      <div className="hidden border-t border-white/10 px-5 py-4 md:block">
        ...
        <div className="mt-2 flex items-center gap-3 text-[11px]">
          <button type="button" disabled={signingOut} onClick={sair} className="text-[#b4fcf1] hover:underline disabled:opacity-50">
            {signingOut ? "saindo…" : "sair"}
          </button>
          <Link href="/auth/reset-password" className="text-[#b4fcf1]/65 hover:text-[#b4fcf1] hover:underline">
            trocar senha
          </Link>
        </div>
      </div>
