...
/**
 * Lista somente contas que ainda existem no Supabase Auth.
 * Perfis históricos de usuários excluídos permanecem no banco para preservar
 * autoria, metas, atividades e demais relacionamentos, mas não aparecem aqui.
 */
export async function listManagedUsers(): Promise<ManagedUsersResult> {
  ...
  if (profileError) {
    return { ok: false, message: `Falha ao carregar perfis: ${profileError.message}`, users: [] }
  }
...
/**
 * Cria a conta sem senha inicial e envia um e-mail para o próprio usuário
 * definir sua senha. Somente o papel gerente é tratado como administrador.
 */
export async function addUser(input: {
  ...
  if (error || !data.user) {
    return { ok: false, message: `Falha ao criar usuário: ${error?.message ?? "erro desconhecido"}` }
  }
  ...
  if (mailError) {
    return {
      ok: true,
      message: `Usuário criado, mas o e-mail para definir a senha não pôde ser enviado agora. Use “Enviar recuperação” para tentar novamente.`,
    }
  }

  return {
    ok: true,
    message: `Usuário ${fullName} adicionado. Enviamos um e-mail para ${email} definir a senha de acesso.`,
  }
}

/** Envia o fluxo oficial de recuperação de senha do Supabase. */
...
/**
 * Remove a identidade de autenticação, mas mantém o perfil como registro
 * histórico para não quebrar atividades, metas, escolas, rotas e auditoria.
 */
export async function deleteUser(userId: string): Promise<UserAdminResult> {
  ...
  const { error: archiveError } = await admin
    .from("profiles")
    .update({ active: false, consultant_tag: null })
    .eq("id", userId)

  if (archiveError) {
    return {
      ok: true,
      message: "A conta de acesso foi excluída. O perfil histórico permaneceu ativo no banco e deve ser revisado pela gerência.",
    }
  }

  return {
    ok: true,
    message: `Usuário ${profile.full_name || profile.email || "selecionado"} excluído. O acesso foi removido e o histórico comercial foi preservado.`,
  }
}
