...
export function rowToEmpresa(row: any, contatos: Contato[], convenio: Convenio | null): Empresa {
  return {
    ...
    proximaAcaoInicio: row.next_action_start_time ? s(row.next_action_start_time).slice(0, 5) : "",
    proximaAcaoFim: row.next_action_end_time ? s(row.next_action_end_time).slice(0, 5) : "",
    proximaAcaoLocal: s(row.next_action_location),
    proximaAcaoOwnerId: row.next_action_owner_id ?? null,
...
export function empresaToRow(e: Empresa, resolveOwner: (e: Empresa) => string | null): Record<string, unknown> {
  return {
    ...
    next_action_start_time: tOrNull(e.proximaAcaoInicio),
    next_action_end_time: tOrNull(e.proximaAcaoFim),
    next_action_location: tOrNull(e.proximaAcaoLocal),
    next_action_owner_id: e.proximaAcaoOwnerId ?? null,
